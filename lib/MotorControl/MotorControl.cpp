...
void MotorControl::startPwm(void) {
  if (*OCRXB > 0) {   // Do only enable if Dutycycle > 0
    ATOMIC_BLOCK(ATOMIC_FORCEON) { PWM_ON; }
    _pwmIsOn = true;
  }
}
...
void MotorControl::stopPwm(void) {
  ATOMIC_BLOCK(ATOMIC_FORCEON) { PWM_OFF; }
  _pwmIsOn = false;
}
...
void MotorControl::setSpeed(uint8_t speed) {
  ...
  *OCRXB = (((PWM_FREQ_PRELOAD * (speed > 100 ? 100 : speed)) + 50) / 100);   // +50 to round up
  ...
  ATOMIC_BLOCK(ATOMIC_FORCEON) {
    *OCRXB =
        (((static_cast<uint32_t>(PWM_FREQ_PRELOAD) * (speed > 100 ? 100 : speed)) + 50) / 100);   // +50 to round up
  }
#endif
}
...
//////////////////////////////////////////////////////////////////////////////
bool MotorControl::isForward() const { return (digitalReadFast(_forwardPin) && !digitalReadFast(_reversePin)); }
...
//////////////////////////////////////////////////////////////////////////////
bool MotorControl::isReverse() const { return (!digitalReadFast(_forwardPin) && digitalReadFast(_reversePin)); }
